...
export modelDataPath=${TOP_DIR}/tests
export txtCntrlFile=${TOP_DIR}/parm/postxconfig-NT-fv3lam.txt
...
if [ ! -d ${DOMAINPATH}/postprd ]; then
  echo "WARNING: DOMAINPATH/postprd, '${DOMAINPATH}/postprd', does not exist. Createing..."
  mkdir -p ${DOMAINPATH}/postprd 
  #exit 1
else
  echo "WARNING: DOMAINPATH/postprd, '${DOMAINPATH}/postprd', existed. Recreateing..."
  rm -rf -p ${DOMAINPATH}/postprd
  mkdir -p ${DOMAINPATH}/postprd
fi
...
curl -O -L https://dl.dropboxusercontent.com/s/3wau7syj1t3ua9i/fv3_lam_output.tar.gz
tar -zxvf fv3_lam_output.tar.gz
rm fv3_lam_output.tar.gz
